...
The dashboard accepts **at most five screenshots**, so the five below are chosen to show five
different jobs rather than five views of one. All are captured from the full-page app — the
popup appears in the wider gallery instead, which only the README uses and which nothing caps.

Each store screenshot carries a 56 px caption band. The app is 1280 px wide, exactly the
screenshot width, so insetting it would shrink the interface; the band sits above a 1280 × 744
capture and keeps every pixel of the UI at 1:1.

| Dashboard field | File | Size |
|---|---|---|
| Store icon | `docs/store/icon-512.png` (or `public/icon/128.png`) | 512 × 512 / 128 × 128 |
| Screenshot 1 | `docs/store/screenshots/01-calculator.png` | 1280 × 800 |
...
| Screenshot 4 | `docs/store/screenshots/04-tools.png` | 1280 × 800 |
| Screenshot 5 | `docs/store/screenshots/05-conflict-checker.png` | 1280 × 800 |
...
2. **Visual planner** — Carve a block into named, colour-coded subnets and watch the utilisation fill
3. **VLSM solver** — Host counts in — an allocation in address order, with the waste each block leaves
4. **Bases and bit mathematics** — DEC, HEX and BIN at a width you choose — and a bit field you can click
5. **Conflict checker** — Duplicates and containment chains, before a merger or a site-to-site VPN

Screenshot 5 is captured in the dark theme, so the listing shows both themes without spending a
slot on a second view of the same screen.

### Not for the listing — `docs/store/gallery/`

Regenerated by the same command, for the README and the website:

| File | What it shows |
|---|---|
| `popup-calculator.png` | The toolbar popup at its real 400 × 600 size |
| `calculator-ipv6.png` | The calculator on an IPv6 block, with the canonical and full forms |
| `projects.png` | The projects list |
| `planner-dark.png` | The planner in the dark theme |
| `tools-dark.png` | The tools page in the dark theme |
| `settings.png` | Settings, including what the extension stores and the delete-all control |

### Support and links

| Field | Value |
|---|---|
| Homepage URL | `https://attv.uk/projects/netcarve.html` |
| Support URL | `https://attv.uk/projects/netcarve.html#feedback` |
| Support email | `dev@attv.uk` |
| Privacy policy URL | `https://attv.uk/netcarve/privacy` |

All three URLs are live. Three things about that set are deliberate:

**Support does not point at GitHub Issues.** Filing one needs a GitHub account,
which a store user is unlikely to have — for them "support" ends at a sign-in
screen. The project page carries a roadmap you can vote on and a report form that
needs no account, and the anchor drops the visitor straight onto the form.
Developers still reach the issue tracker from the Links row on that page.

**Homepage is the project page, not the site root.** In a store listing "homepage"
means the page about *this extension*, not the page about its author.

**The host is `attv.uk`, not `www.attv.uk`.** Both resolve, but the pages'
`<link rel="canonical">` is the apex form, so the listing and the canonical should
agree. The `www` form stays where it belongs — in the credit footer's link text,
which is a house rule of its own and is unrelated to these fields.
